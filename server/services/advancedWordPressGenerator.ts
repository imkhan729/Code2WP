...
class ${this.getFunctionPrefix()}_Nav_Walker extends Walker_Nav_Menu {
  ...
  private async generateTemplateFiles(themeFolder: JSZip, analysis: AnalysisReport, diagnostics: DiagnosticsReport): Promise<void> {
    // Generate individual page templates for each HTML page with proper routing
    for (const page of analysis.pages) {
      if (page.filename !== analysis.homepage) {
        // Create page template with route mapping
        const templateName = this.getTemplateNameFromPage(page.filename);
        const pageTemplate = this.generatePageTemplateWithRouting(page, analysis);
        themeFolder.file(`page-${templateName}.php`, pageTemplate);
        
        // Generate custom post template if it's blog-like
        if (page.isBlogLike) {
          const postTemplate = this.generateCustomPostTemplate(page, analysis);
          themeFolder.file(`single-${templateName}.php`, postTemplate);
        }
      }
    }

    // Generate category and archive templates if blog pages exist
    if (analysis.blogPages.length > 0) {
      const categoryTemplate = this.generateCategoryTemplate(analysis);
      themeFolder.file('category.php', categoryTemplate);
      
      const tagTemplate = this.generateTagTemplate(analysis);
      themeFolder.file('tag.php', tagTemplate);
    }

    // Generate template parts for reusable sections
    await this.generateTemplateParts(themeFolder, analysis);
    
    // Generate URL rewrite rules for clean routing
    const routingRules = this.generateRoutingRules(analysis);
    themeFolder.file('includes/routing-rules.php', routingRules);
  }
...
<body>
  ...
  // Enhanced methods for comprehensive website support
  private getTemplateNameFromPage(filename: string): string {
    // Convert filename to valid WordPress template name
  ...
  private generatePageTemplateWithRouting(page: any, analysis: AnalysisReport): string {
    const $ = cheerio.load(page.content);
    
    // Remove header and footer
    $('header, footer, nav, .header, .footer, .nav').remove();
    
    // Convert internal links to WordPress permalinks
    this.convertInternalLinks($, analysis);
...
<?php get_footer(); ?>`;
  ...
  private generateCustomPostTemplate(page: any, analysis: AnalysisReport): string {
    const $ = cheerio.load(page.content);
    
    // Extract article/post content
    const articleContent = $('article, .post, .blog-post').first().html() || $('body').html() || '';
    
    return `<?php
/**
 * Single post template for ${page.title}
 * Generated from: ${page.filename}
 */
...
<?php get_footer(); ?>`;
  ...
  private generateCategoryTemplate(analysis: AnalysisReport): string {
    const blogPage = analysis.blogPages[0];
    if (!blogPage) return this.getDefaultCategoryTemplate();
...
<?php get_footer(); ?>`;
  ...
  private generateTagTemplate(analysis: AnalysisReport): string {
    return `<?php
/**
...
<?php get_footer(); ?>`;
  ...
  private async generateTemplateParts(themeFolder: JSZip, analysis: AnalysisReport): Promise<void> {
    const partsFolder = themeFolder.folder('template-parts');
    if (!partsFolder) return;

    // Generate navigation part if exists
    if (analysis.navigation) {
      const navPart = this.generateNavigationPart(analysis.navigation);
      partsFolder.file('navigation.php', navPart);
    }

    // Generate content parts for reusable sections
    const contentPart = `<?php
/**
...
<article id="post-<?php the_ID(); ?>" <?php post_class(); ?>>
    ...
    partsFolder.file('content-page.php', contentPart);
  }

  private generateNavigationPart(navigation: NavigationInfo): string {
    return `<?php
/**
...
<nav class="main-navigation" role="navigation">
  ...
  private generateRoutingRules(analysis: AnalysisReport): string {
    const rules = analysis.pages
      .filter(page => page.filename !== analysis.homepage)
      .map(page => {
        const slug = this.getTemplateNameFromPage(page.filename);
        return `add_rewrite_rule('^${slug}/?$', 'index.php?pagename=${slug}', 'top');`;
      })
      .join('\n    ');
 ...
 */
function ${this.getFunctionPrefix()}_add_rewrite_rules() {
    ${rules}
    
    // Flush rewrite rules on theme activation
    flush_rewrite_rules();
}
add_action('init', '${this.getFunctionPrefix()}_add_rewrite_rules');

/**
 * Add query vars
 */
function ${this.getFunctionPrefix()}_add_query_vars($vars) {
    $vars[] = 'custom_page';
    return $vars;
}
add_filter('query_vars', '${this.getFunctionPrefix()}_add_query_vars');

/**
 * Template redirect for custom pages
 */
function ${this.getFunctionPrefix()}_template_redirect() {
    global $wp_query;
...
add_action('template_redirect', '${this.getFunctionPrefix()}_template_redirect');
?>`;
  }
  ...
  private convertInternalLinks($: cheerio.CheerioAPI, analysis: AnalysisReport): void {
    $('a[href]').each((_, link) => {
      ...
      if (href && !href.startsWith('http') && !href.startsWith('#') && 
        ...
        wpHref = wpHref.replace(/\.html?$/i, '');
        
        // Clean up the path
        wpHref = wpHref.replace(/^\.\//, '').replace(/\/+/g, '/');
        
        // Convert to WordPress permalink structure
        if (wpHref === 'index' || wpHref === '') {
          wpHref = '<?php echo home_url(); ?>';
        } else {
          wpHref = `<?php echo home_url('/${wpHref}/'); ?>`;
        }
        
        $link.attr('href', wpHref);
      }
    });
  }

  private processAssetPaths($: cheerio.CheerioAPI): void {
    // Process image sources
    $('img[src]').each((_, img) => {
      const $img = $(img);
      const src = $img.attr('src');
      if (src && !src.startsWith('http') && !src.startsWith('data:')) {
        const wpSrc = `<?php echo get_template_directory_uri(); ?>/${src.replace(/^\.\//, '')}`;
        $img.attr('src', wpSrc);
      }
    });
      ...
      const updatedStyle = style.replace(
        ...
        (match, url) => {
          if (!url.startsWith('http') && !url.startsWith('data:')) {
            return `url('<?php echo get_template_directory_uri(); ?>/${url.replace(/^\.\//, '')}')`;
          }
          return match;
        }
      );
